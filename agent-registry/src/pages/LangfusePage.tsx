...
import { Layers, Plus, Trash2, CheckCircle2, Loader2, AlertCircle } from "lucide-react";
import { useData } from "../context/DataContext";
import type { LangfuseInstance } from "../types";

async function testLangfuse(inst: Omit<LangfuseInstance, "id" | "userId">): Promise<string> {
  const credentials = btoa(`${inst.publicKey}:${inst.secretKey}`);
  const res = await fetch(
    `${inst.hostUrl.replace(/\/$/, "")}/api/public/projects`,
...
export function LangfusePage() {
  const { langfuseInstances, addLangfuse, removeLangfuse } = useData();

  const [name, setName] = useState("");
  const [hostUrl, setHostUrl] = useState("https://cloud.langfuse.com");
  ...
  const reset = () => {
    ...
    setPublicKey("");
    setSecretKey("");
    setHostUrl("https://cloud.langfuse.com");
    setTestResult(null);
  };
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      const msg = await testLangfuse({ name, hostUrl, publicKey, secretKey });
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Langfuse instances</h1>
      </div>
      <p className="text-sm text-gray-500 mb-6">Connect Langfuse Cloud or a self-hosted instance. Agents are pinned to connected instances.</p>
      ...
      {langfuseInstances.length === 0 ? (
        <div className="border border-gray-200 rounded-xl bg-white px-5 py-8 text-center text-sm text-gray-500">
          No instances yet.
        </div>
      ) : (
        <div className="space-y-2">
          {langfuseInstances.map((inst) => (
            <div key={inst.id} className="bg-white border border-gray-200 rounded-xl px-5 py-4 flex items-center justify-between">
              <div>
                <p className="font-medium text-gray-900 text-sm">{inst.name || "Langfuse Cloud"}</p>
                <p className="text-xs text-gray-500 mt-0.5">{inst.hostUrl}</p>
                <p className="text-xs font-mono text-gray-400 mt-0.5">{inst.publicKey}</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={async () => await removeLangfuse(inst.id)}
